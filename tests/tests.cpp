...
namespace Tests
{
	...
	TEST_CLASS(Symbols)
	{
		...
		TEST_METHOD(Functions)
		{
			...
			Assert::AreNotEqual((std::string)"-1", calc("rand()"));
			Assert::AreNotEqual((std::string)"-1", calc("rand()+1"));
			Assert::AreNotEqual((std::string)"-1", calc("1+rand()"));
			Assert::AreNotEqual((std::string)"0.5", calc("round(rand())"));
